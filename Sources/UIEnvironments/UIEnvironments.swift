...
@MainActor public class UIEnvironments {
    ...
    ///
    /// It walks the responder chain and merges every specified override, with
    /// the nearest responder winning, reflecting the same fallback resolution
    /// used by the subscript (so it does not consult natively bridged trait
    /// values).
    ///
